...
import { addTeacher, getAllUser } from "../services/userManagement.service";
...
const InstructorManagementPage: React.FC = () => {
  ...
  const handleAddInstructor = async () => {
    ...
    if (!user.name) {
      alert("Tên giảng viên là bắt buộc!");
      return;
    }
    if (!user.age) {
    ...
    try {
      const response = await addTeacher([user]);
  ...
  return (
    <div className="bg-gray-50 w-screen h-screen flex relative overflow-hidden">
      ...
      <div className="flex-1 p-6 ml-[40px] mr-[30px] overflow-auto scrollbar-hide">
        <h1 className="text-2xl font-semibold mb-6">Quản lý giảng viên hướng dẫn</h1>
