...
import { CouncilDetailResponse, UpdateCouncilRequest } from "../types/council.types";
...
const CouncilDetail: React.FC = () => {
  ...
  const [deleteBoardMemberList, setDeleteBoardMemberList] =
    useState<CRUDBoardMember[]>([]);

  const [deleteDefenseScheduleList, setDeleteDefenseScheduleList] =
    useState<CRUDDefenseSchedule[]>([]);
  ...
  const fetchCouncilDetail = async (id: number) => {
    try {
      const res: ApiResponse<CouncilDetailResponse | string> =
        await getCouncilDetail(id);
      if (res.code === 200 && typeof res.result !== "string") {
        const data = res.result;
        setCouncilName(data.name);
        ...
        setBoarMemberList(data.boardMemberList);
        setDefenseScheduleList(data.defenseScheduleList);
      } else if (res.code === 400 && typeof res.result === "string") {
        alert(`Lỗi khi lấy thông tin hội đồng: ${res.result}`);
      }
    } catch (error) {
      console.error(error);
      alert(`Lỗi khi lấy thông tin hội đồng: ${error}`);
    }
  };
  ...
  const handleUpdateCouncil = async () => {
    ...
    const boardMemberChange = [
      ...(deleteBoardMemberList ?? []),
      ...(boardMemberList ?? []),
    ];

    const defenseScheduleChange = [
      ...(deleteDefenseScheduleList ?? []),
      ...(defenseScheduleList ?? []),
    ];

    const request: UpdateCouncilRequest = {
      id: Number(id),
      name: councilName,
      ...
      boardMemberList: boardMemberChange,
      scheduleRequests: defenseScheduleChange,
    };

    try {
      const res = await updateCouncil(request);
      if (res.code === 200 && typeof res.result !== "string") {
      ...
      console.error(error);
      alert("Đã xảy ra lỗi khi cập nhật hội đồng.");
    }
  };

  useEffect(() => {
    fetchCouncilDetail(Number(id));
          ...
          <div className="flex items-center mb-3 gap-5">
            <div className="flex-1">
              ...
              <a
                ...
                className="block w-full border border-gray-400 rounded px-3 py-2 text-blue-600 hover:underline"
              >
                {decodeFileName(fileUrl)}
              </a>
            </div>
          </div>
        </div>
                      ...
                      <button
                        onClick={() => {
                          const updatedBoardMember = boardMemberList.filter(
                            (member) =>
                              member.user.idNum !== boardMember.user.idNum
                          );
                          setBoarMemberList(updatedBoardMember);
                          boardMember.action = ActionTypes.DELETE;
                          deleteBoardMemberList?.push(boardMember);
                        }}
                ...
                {defenseScheduleList?.map((defenseSchedule, idx) => (
                  <tr key={idx} className="cursor-pointer border-t">
                    ...
                    <td className="py-2 px-4">
                      <button
                        onClick={() => {
                          const updateDefenseScheduleList =
                            defenseScheduleList.filter(
                              (ds) =>
                                ds.topic.idNum !== defenseSchedule.topic.idNum
                            );
                          setDefenseScheduleList(updateDefenseScheduleList);
                          defenseSchedule.action = ActionTypes.DELETE;
                          deleteDefenseScheduleList?.push(defenseSchedule);
                        }}
                        className="border-blue-900 border-2 hover:bg-red-700 hover:text-white px-4 py-2 rounded"
                      >
                        Xóa
